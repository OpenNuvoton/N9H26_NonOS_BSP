...
#define clr_IRQ0_GPB1 outpw(REG_IRQTGSRC0,inpw(REG_IRQTGSRC0) &0x00020000); 
...
void vu_SPI_WRITE(int len,char *pSrc)
{
	...
	outpw(REG_SPI1_SSR, inpw(REG_SPI1_SSR) | 0x01);	// CS0 _LOW
	tmp_src[0]=*pSrc;
	tmp_src[1]=*(pSrc+1);
	tmp_src[2]=*(pSrc+2);
	tmp_src[3]=*(pSrc+3);
	spiWrite(SPI_port, SPI_32BIT, len, pSrc);
	outpw(REG_SPI1_SSR, inpw(REG_SPI1_SSR) & 0xfffffffe);	// CS0 _HIGH
...
void GPIO_GPAR_ENABLE()
{
		...
		DBG_PRINTF("enable GPB1 as interrupt to notify (1)finish of TX (2)Data in buffer from controller\n");
		//set GPA0 input 'b0:input ; 'b1:output;
		outpw(REG_GPIOB_OMD,   inpw(REG_GPIOB_OMD & ( ~(0x1<<1) ) ));
		//set GPA0 Bit Pull-up Resistor Enable 'b0= disable ;'b1= enable
		outpw(REG_GPIOB_PUEN , inpw(REG_GPIOB_PUEN) | (0x1<<1) ); 
		...
		*/
		//set GPB1 rising edge trigger
		outpw(REG_IRQENGPB , ((inpw(REG_IRQENGPB)&(~(0x1<<1))) | ((0x1<<16<<1))));
	
		//set GPB1 group IRQ0 interrupt 2'b00=IRQ0 2'b01=IRQ1 2'b10=IRQ2 2'b11=IRQ3
		outpw(REG_IRQSRCGPB , inpw(REG_IRQSRCGPB) & ~((0x3)<<1));
