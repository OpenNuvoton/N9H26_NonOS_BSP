...
#ifdef __UPLL_240__
    sysprintf("SPI Loader (UpdateLoader) start 240MHz (%s)\n", DATE_CODE);
#elif defined __UPLL_264__
    sysprintf("SPI Loader (UpdateLoader) start 264MHz (%s)\n", DATE_CODE);
#else
...
#else
                spiFlashFastReadQuads(g_UpdateStartBlock * 0x10000, 512, (UINT32*)pImageList);
#endif
...
#else
                    spiFlashFastReadQuads(g_UpdateStartBlock * 0x10000 + offset, 4, (UINT32*)pImageList);
#endif
...
#else
                spiFlashFastReadQuads(start_addr, fileLen, (UINT32*)executeAddr);
#endif
