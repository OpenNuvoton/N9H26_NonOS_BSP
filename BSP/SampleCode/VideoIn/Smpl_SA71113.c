...
UINT32 Smpl_SA71113_VGA(UINT8* pu8FrameBuffer0, UINT8* pu8FrameBuffer1, UINT8* pu8FrameBuffer2)
{
	...
	{	
		if(i==0){
			//0: Both fields are disabled. 1: Field 1 enable. 2: Field 2 enable. 3: Both fields are enable
			//0: CCIR601.	1: CCIR656 	
			pVin->SetFieldDetection(0, 0);
			pVin->SetInputType(3, (E_VIDEOIN_TYPE) mode, 0);						//swap?
		}else if(i==1){
			pVin->SetFieldDetection(0, 0);
			pVin->SetInputType(3,  (E_VIDEOIN_TYPE) mode, 1);						//swap?		
		}else if(i==2){
			#if 0
			videoIn0_SetFieldDetection(0, 1);	
			pVin->SetInputType(3,  (E_VIDEOIN_TYPE) mode, 0);	
			#endif		
		}else if(i==3){
			#if 0
			videoIn0_SetFieldDetection(0, 1);	
			pVin->SetInputType(3, (E_VIDEOIN_TYPE) mode, 1);	
			#endif		
		}else if(i==4){
